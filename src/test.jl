    ...
    for u in [10;20]
        ...
        for (m1,l1) in [(ceil(Int64, n/5),0);(ceil(Int64, n/5),ceil(Int64, n/5))]
            ...
            for k_Pu in [1;2]
                ...
                println("**Semidefinite relaxation based on Putinar's Positivstellensatz**")
                println("Relaxation order: k=",k)
                println("Maximal matrix size: ",binomial(u+k+1,k))
                if binomial(u+k+1,k)>250
                ...
                if k_Pu==2
                    ...
                    @time RelaxSparse_without_multiplier(n,m,l,lmon_g,supp_g,coe_g,lmon_h,supp_h,coe_h,lmon_f,supp_f,coe_f,dg,dh,s,k,assign="min",alg="MD",minimize=true,solver="Mosek",comp_opt_sol=false,L=ones(Float64,150))
