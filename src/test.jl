...
function test_compare_with_sign_sym()
    ...
    for n in [100,200]
        ...
        a=ones(Float64,n)/n
        f=-sum((x-a).^2)
        g=[1.0-sum(x)];m=length(g)
        l=0
        h=Vector{Polynomial{true,Float64}}(undef,l);l=length(h)
        ...
        @time begin
            opt_val,sol,data=TSSOS.tssos_first(pop,x,k,numeq=l,TS="block",solver="Mosek");
